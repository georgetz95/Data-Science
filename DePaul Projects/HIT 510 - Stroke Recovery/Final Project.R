...
library(huxtable)
library(xtable)
gt_table_F %>% as_gt() %>% gt::as_latex() %>% write_clip()
# as_kable_extra(gt_table_F, format = "latex") %>% write_clip()
as_kable(gt_table_F, format = "latex", longtable=T, booktabs=T, escape=T) %>% write_clip()
as_hux_table(tbl) %>% huxtable::to_latex() %>% write_clip()
kbl(gt_table_F, booktabs = T, longtable=T, format = "latex") #%>% write_clip()
...
## -------------------------------------------------------------------------------------------------------------------------------
tbl_3mo <- tbl_regression(lm_ipw_dr, exponentiate = FALSE, label = list(support_high_F ~ "High Support"), intercept=T) %>%
...
kruskal.test(cate ~ fim_quartile, data = df_Y) %>% 
  ...
  kable(format = "latex", booktabs = F, longtable=T,
        caption = "Kruskal–Wallis test comparing CATE distributions across baseline FIM quartiles.",
