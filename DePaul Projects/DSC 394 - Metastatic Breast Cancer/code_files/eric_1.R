...
model {
  ...
  # Priors
  beta0 ~ dnorm(0, 0.001)
  beta_PC1 ~ dt(0, 1, 1)  # Cauchy prior for PC1
  
  # Priors for patient_age
  beta_age ~ dnorm(59.27, 13.21888)
  
  # Priors for patient_state_mean
  beta_state ~ dnorm(96.53, 10.60562)
  
  # Priors for breast_cancer_diagnosis_code_mean_category
  beta_bcd[1] ~ dnorm(54.34868, 79.64735) # Low category
  beta_bcd[2] ~ dnorm(59.05892, 82.03951) # Medium category
  beta_bcd[3] ~ dnorm(205.21936, 102.19856) # High category
  
  # Priors for metastatic_cancer_diagnosis_code_mean_category
  beta_mcd[1] ~ dnorm(78.07984, 111.7566) # Low category
  beta_mcd[2] ~ dnorm(90.86825, 102.5487) # Medium category
  beta_mcd[3] ~ dnorm(115.13401, 121.5643) # High category
...
samples_all_betas <- coda.samples(model, variable.names = c("beta0", "beta_PC1", "beta_age", "beta_state", paste0("beta_bcd[", 1:3, "]"), paste0("beta_mcd[", 1:3, "]")), n.iter = iterations)
...
# Sampling for y from each chain, n.iter results in n.chains * n.iter for 20 rows, 5 per chain
samples_y <- coda.samples(model, variable.names = c("y"), n.iter = 5)

# Extract the predicted values
predicted_values <- as.matrix(samples_y[, , "y"], dimnames = FALSE)

# Extract the column means
mean_predictions <- colMeans(predicted_values, na.rm = TRUE)
mean_predictions <- as.matrix(mean_predictions)

# Param samples
parameter_samples <- as.matrix(samples_all_betas)

# Calculate the mean of the posterior samples for each parameter
mean_parameters <- colMeans(parameter_samples)

# Number of posterior samples
n_samples <- nrow(parameter_samples)

# Initialize matrix to store predicted values
predicted_values_matrix <- matrix(0, nrow = N, ncol = n_samples)

# Loop through each posterior sample in the MCMC chain
for (j in 1:n_samples) {
  # Extract the current posterior sample
  current_sample <- parameter_samples[j, ]

  # Loop through each row in train_pca
  for (i in 1:N) {
    # Extract predictor values for the current row
    PC1 <- train_pca$PC1[i]
    patient_age <- train_pca$patient_age[i]
    patient_state_mean <- train_pca$patient_state_mean[i]
    bcd_category <- train_pca$breast_cancer_diagnosis_code_mean_category[i]
    mcd_category <- train_pca$metastatic_cancer_diagnosis_code_mean_category[i]

    # Calculate linear predictor using the current posterior sample
    mu <- current_sample["beta0"] +
      current_sample["beta_PC1"] * PC1 +
      current_sample["beta_age"] * patient_age +
      current_sample["beta_state"] * patient_state_mean +
      current_sample[paste0("beta_bcd[", bcd_category, "]")] +
      current_sample[paste0("beta_mcd[", mcd_category, "]")]

    # Store the predicted value for the current row and posterior sample in the matrix
    predicted_values_matrix[i, j] <- mu
  }
}

# Calculate the mean predicted values across all posterior samples
predicted_values_mean <- rowMeans(predicted_values_matrix)
...
comparison_df <- data.frame(
  ...
  Predicted = predicted_values_mean
)
...
rmse_bayes <- sqrt(mean((train_pca$metastatic_diagnosis_period - predicted_values_mean)^2))
mae_bayes <- mean(abs(train_pca$metastatic_diagnosis_period - predicted_values_mean))
mse_bayes <- mean((train_pca$metastatic_diagnosis_period - predicted_values_mean)^2)
